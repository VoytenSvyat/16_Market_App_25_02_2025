...
function addProductToDom(item) {
  ...
  // HW Advanced(добавить кнопки +  и - для изменения количества товара)
  li.innerHTML = `
    <span>
...
// 2 способ кнопки удаления deleteBtn через id в самой кнопке button
...
function deleteProductFromDom(itemName) {
  ...
  productsList.removeChild(li); // li.remove() - удаление елемента li из DOM((обьектная модель документа)
}
